// Copyright (c) FIRST and other WPILib contributors.
// Open Source Software; you can modify and/or share it under the terms of
// the WPILib BSD license file in the root directory of this project.

package frc.robot.subsystems;

import edu.wpi.first.wpilibj2.command.Command;
import edu.wpi.first.wpilibj2.command.SubsystemBase;
import com.revrobotics.CANSparkMax;
import com.revrobotics.CANSparkBase;
import com.revrobotics.CANSparkFlex;
import com.revrobotics.RelativeEncoder;
import com.revrobotics.SparkPIDController;
import com.revrobotics.CANSparkLowLevel.MotorType;
import com.revrobotics.CANSparkLowLevel.PeriodicFrame;

import edu.wpi.first.wpilibj.smartdashboard.SmartDashboard;
import edu.wpi.first.wpilibj.DigitalInput;



public class WristInfeed extends SubsystemBase {
  /*  */
  public CANSparkFlex InfeedMotor1=new CANSparkFlex(13, MotorType.kBrushless); //Used to load up into shooter
  // public CANSparkMax InfeedMotor2=new CANSparkMax(26, MotorType.kBrushless); //For under the bumper, use later44

  public RelativeEncoder InfeedEncoder = InfeedMotor1.getEncoder();
  public SparkPIDController InfeedPIDController = InfeedMotor1.getPIDController();

  public double kP = 5.4, //.0014, 1.4
                kI = 0.000001,
                kD = 0,
                kIz = 0,
                kFF = 5.6;  //.16,1.6
              


  public WristInfeed() { 
    // InfeedMotor2.follow(InfeedMotor1, true);

    InfeedPIDController.setP(kP);
    InfeedPIDController.setI(kI);
    InfeedPIDController.setD(kD);
    InfeedPIDController.setIZone(kIz);
    InfeedPIDController.setFF(kFF);
    InfeedPIDController.setOutputRange(-0.1, 0.1);
    InfeedPIDController.setFeedbackDevice(InfeedEncoder);
    InfeedPIDController.setSmartMotionAllowedClosedLoopError(0.005,0);

     InfeedMotor1.setPeriodicFramePeriod(PeriodicFrame.kStatus1,20);
     InfeedMotor1.setPeriodicFramePeriod(PeriodicFrame.kStatus2,20);
    SmartDashboard.putNumber("Infeed Speed", 1);//0.7
    SmartDashboard.putNumber("Infeed Stop Speed", 0.5);

    // SmartDashboard.putNumber("Infeed Stop kP", kP);
    // SmartDashboard.putNumber("Infeed Stop kI", kI);
    // SmartDashboard.putNumber("Infeed Stop kD", kD);
    // SmartDashboard.putNumber("Infeed Stop kFF", kFF);

   }

  /**
   * Runs the intake for the infeed subsystem (sucks notes into rollers)
   *
   * @return a command that runs once, engages intake
   */
  public Command SmartIntake() {
    // double speed = SmartDashboard.getNumber("Infeed Speed", 0.3);
   return runOnce(()-> InfeedMotor1.set(SmartDashboard.getNumber("Infeed Speed", 1))).withName("Run shooter");
  }

  public Command Intake(double speed) {
    // double speed = SmartDashboard.getNumber("Infeed Speed", 0.3);
   return runOnce(()-> InfeedMotor1.set(speed)).withName("Run shooter");
  }

  /**
   * Runs the intake for the infeed subsystem (sucks notes into rollers)
   *
   * @return a command that runs repeatedly, engages intake
   */
  public Command ContinuousIntake(double speed) {
    // double speed = SmartDashboard.getNumber("Infeed Speed", 0.3);
  //  return run(()-> InfeedMotor1.set(SmartDashboard.getNumber("Infeed Stop Speed", 0.7))).withName("Run shooter");
    return Intake(speed).repeatedly();
  }
   
  public Command ContinuousSmartIntake() {
    // double speed = SmartDashboard.getNumber("Infeed Speed", 0.3);
  //  return run(()-> InfeedMotor1.set(SmartDashboard.getNumber("Infeed Stop Speed", 0.7))).withName("Run shooter");
    return SmartIntake().repeatedly();
  }

  /**
   * Runs the outtake for the infeed subsystem (sucks notes out of rollers)
   *
   * @return a command that runs once, engages outake
   */
  public Command Outtake() {
    // double speed = SmartDashboard.getNumber("Infeed Speed", 0.3);
   return runOnce(()-> InfeedMotor1.set(-1*SmartDashboard.getNumber("Infeed Speed", 0.7))).withName("Run shooter");
  }



  public Command StopInfeed() {
   return runOnce(()-> InfeedMotor1.set(0.0));
  }
public Command PIDStop() {
   return run(()->InfeedPIDController.setReference(0, CANSparkBase.ControlType.kVelocity)) ;
  }



  @Override
  public void periodic() {
    // This method will be called once per scheduler run
    SmartDashboard.putNumber("Infeed Encoder Rpm", InfeedMotor1.getEncoder().getVelocity());


  }

  @Override
  public void simulationPeriodic() {
    // This method will be called once per scheduler run during simulation
  }
}
